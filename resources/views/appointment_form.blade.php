            ...
            <div class="col-xs-12 col-md-10 col-xl-8 mb-5">
                <div class="card">
                    <div class="card-body m-0 p-0">
                        <form method="POST" action="{{ route('make_appointment') }}" id="id">
                            @csrf
                            ...
                            <div class="px-5 py-4">
                                ...
                                <div
                                    @if (auth()->user()->role_id == 4 || auth()->user()->role_id == 5) class='row mb-3' @else class='row mb-3' @endif>
                                    <label for="clinic" class=" col-form-label text-md-right mt-0 pt-0">Clinic <span
                                            class="text-danger">*</span></label>
                                    <div class="col">
                                        <!-- added by Rizal for slide 10:  @if (auth()->user()->role_id == 1) disabled @endif -->
                                        <select @if (auth()->user()->role_id == 1) disabled @endif class="form-select @error('clinic') is-invalid @enderror" id="clinic"
                                    ...
                                    <div class="col">
                                        ...
                                        <div class="row">
                                            <div class="col-md-12 col-sm-12 col-lg-6 mb-2">
                                                <label for="code"
                                                    class="col-form-label text-md-right pt-0 fw-bold">zPatient's Code <span
                                                        class="text-danger">*</span></label>
                                ...
                                <!-- added by Rizal for slide 10 point 2 -->
                ...
                $('.weekday_slot option').each(function() {
                    if (this.value !== '' && moment().format('DD-MM-YYYY') === selected_date) {
                        ...
                        if (moment.duration(times_selected_1.diff(currenthour)).asHours() < 2) {
                            $(this).attr("disabled", true).css("color","#f2f2f2"); /*** added by Rizal for slide 13 */
                        }
                    /*** added by Rizal for slide 13 */
                    }else{
                        $(this).attr("disabled", false).css("color","#000000");
                    }
                    /*** end added by Rizal */
                });
        ...
        $(document).ready(function() {
           ...
           /* added by Rizal for slide 10 point 2 */
            $("#existing_patient").change(function(){
                $("#myDiv").hide();
                console.log('ayam '+ $("#existing_patient").val());
            
                $.ajax({  
                type: "POST",
                url: "{{ route('patientCode') }}",
                data: {id : $("#existing_patient").val()},
                success: function(response){
                    console.log(response);
                    $("#myDiv").html(response).show();
                },
